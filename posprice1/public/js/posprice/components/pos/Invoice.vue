  ...
  methods: {
    ...
    add_item(item) {
      ...
      if (index === -1 || this.new_line) {
        ...
        { 
         this.get_item_price_list_rate(new_item,this.get_price_list(),new_item.uom);
         new_item.stock_qty = item.qty*item.conversion_factor;
         this.calc_item_price(new_item);
         new_item.standart_selling_price = this.get_standard_price_list_rate(item, "Standard Selling", item.uom);
         this.update_item_detail_cr(new_item);
    ...
    get_new_item(item) {
      ...
      new_item.standart_selling_price = item.standart_selling_price;
      if (this.pos_profile.offline){
      new_item.item_uoms = item.item_uoms;
      new_item.item_prices = item.item_prices;
      new_item.rate = this.get_item_price_list_rate(item, this.get_price_list(), item.uom);
    ...
    //For Offline mode
    get_item_price_list_rate(item, price_list, uom) {
    if (!item || !item.item_prices || item.item_prices.length === 0) {
        return null; // No prices available for this item
    }

    // Filter through the item prices to find the match
    const matched_price = item.item_prices.find(price => 
        price.price_list === price_list && price.uom === uom
    );
    if (!matched_price) {
      frappe.throw(`No price found for Price List: '${item.item_code}' and '${price_list}' and UOM: '${uom}'`);
    }
    item.rate = matched_price.price_list_rate;
    item.price_list_rate = matched_price.price_list_rate;
    // Return the price_list_rate if found, otherwise null
    return matched_price ? matched_price.price_list_rate : null;
},
//For Offline mode
get_standard_price_list_rate(item, price_list, uom) {
    if (!item || !item.item_prices || item.item_prices.length === 0) {
        return null; // No prices available for this item
    }
    ...
    get_invoice_doc() {
      ...
      doc.error_submit = this.error_submit;
      return doc;
    },
    ...
    get_invoice_items() {
      ...
      this.items.forEach((item) => {
        const new_item = {
          ...
          bonus: flt(item.qty)* flt(item.bonus),
          standart_selling_price: item.standart_selling_price,
        };
    ...
    show_payment() {
      ...
      localStorage.setItem('paymentWindow', 'true');
      if (!this.customer) {
        evntBus.$emit('show_mesage', {
 ...
 //     invoice_doc.error_submit = this.invoice_doc.error_submit;
      ...
      this.items.forEach((item) => {
        if (this.stock_settings.allow_negative_stock != 1) {
          ...
          if (
            ...
            //this.error_submit = "Error";
         
            evntBus.$emit('show_mesage', {
              text: __(
                `Имеющегося количества '{0}' для товара '{1}' недостаточно`,
          ...
          //Change: Adding Projected qty to reserve what was ordered from Agents.
          // if (
          //   ( (item.is_stock_item && item.stock_qty > item.projected_qty ))
          // ) {
          //   evntBus.$emit('show_mesage', {
          //     text: __(
          //       `Вы можете продовать '{0}' количество от товара '{1}' . Товар может быть заразервирован`,
          //       [item.projected_qty, item.item_name ]
          //     ),
          //     color: 'error',
          //   });
          //   value = false;
      ...
      frappe.call({
        ...
        callback: function (r) {
          if (r.message) {
            ...
            item.standart_selling_price = data.standart_selling_price;
    ...
    update_item_detail_cr(item) {
      const vm = this;
      var doc = this.get_invoice_doc();
      doc.items= [];
      frappe.call({
        method: 'posawesome.posawesome.api.posapp.get_item_detail_new',
        args: {
          warehouse: this.pos_profile.warehouse,
          doc: doc,
          price_list: this.pos_profile.price_list,
   ...
   //         item.conversion_factor = data.conversion_factor;
    ...
    calc_uom(item, value) {
      ...
      {
        this.get_item_price_list_rate(item,this.get_price_list(),item.uom);
        item.stock_qty = item.qty*item.conversion_factor;
        item.standart_selling_price = this.get_standard_price_list_rate(item, "Standard Selling", item.uom);
        // this.update_item_detail_cr(item);
    ...
    //custom change 
    //TODO: Make custom JS for Custom code 
    shortSave(e){
    ...
    shortSelectUOM(e) {
      ...
      if (e.key === 'F2' ) {
        e.preventDefault();
        this.changeUOMBlock('Блок');
      }
    }
    },
    changeUOM(new_uom){
      if(!this.pos_profile.offline){
      frappe.show_progress('Update..', 90, 100, 'Please wait', hide_on_completion = true);}
      //uom = this.expanded[0].item_uoms.find((element) => element.uom === new_uom );
      if ( this.pos_profile.company == "Chynar"){
        uom = this.expanded[0].item_uoms.find((element) => element.uom !== this.expanded[0].uom);
      }
      else {
      uom = this.expanded[0].item_uoms.find((element) => element.uom === new_uom || element.uom == 'Кол-во' || element.uom == 'кг');
    }
      if (uom){ 
        uom = uom.uom;
        evntBus.$emit('show_mesage', {
        ...
        this.expanded[0].uom = uom;
        this.calc_uom(this.expanded[0], uom);
        if(!this.pos_profile.offline){
        frappe.show_progress('Update..', 101, 100, 'Please wait', hide_on_completion = true);}
    }},
    changeUOMBlock(new_uom){
      if(!this.pos_profile.offline){
      frappe.show_progress('Update..', 90, 100, 'Please wait', hide_on_completion = true);}
      //uom = this.expanded[0].item_uoms.find((element) => element.uom === new_uom );
      if ( this.pos_profile.company == "Chynar"){
        ...
        var uomsSet = new Set(['Blok', 'Blok(1)', 'Блок', 'Koropka', 'Halta','Latok','Baçok','Пачка']);

      // Assume this.expanded[0].item_uoms is an array of objects where each object has a property 'uom'
        var item_uoms = this.expanded[0].item_uoms; // or however you access item_uoms

        // Initialize variable to store the found UOM
        var foundUom = null;

        // Loop through item_uoms and check if each uom is in the uomsSet
            for (var i = 0; i < item_uoms.length; i++) {
              if (uomsSet.has(item_uoms[i].uom)) {
                uom = item_uoms[i];
                console.log('Found UOM:', foundUom);
  ...
  created() {
    ...
    evntBus.$on('update_invoice_items', () => {
    ...
    document.addEventListener('keydown', this.shortFocusRate.bind(this));
    document.addEventListener('keydown', this.shortSelectDiscount.bind(this));
    document.addEventListener('keydown', this.shortSelectUOM.bind(this));
    document.addEventListener('keydown', this.shortSave.bind(this));
    document.addEventListener('keydown', this.shortDraft.bind(this));
  },
  destroyed() {
    document.removeEventListener('keydown', this.shortFocusRate);
    document.removeEventListener('keydown', this.shortCloseFirstItem);
    document.removeEventListener('keydown', this.shortOpenPayment);
    document.removeEventListener('keydown', this.shortDeleteFirstItem);
    document.removeEventListener('keydown', this.shortOpenFirstItem);
    document.removeEventListener('keydown', this.shortSelectDiscount);
    document.removeEventListener('keydown', this.shortSelectUOM);
    document.removeEventListener('keydown', this.shortDraft);
  },
  watch: {
    customer() {
      this.close_payments();
    ...
    expanded(data_value) {
      ...
      { if(!data_value[0].item_uoms){
         var all_items = JSON.parse(localStorage.getItem('items_storage'));
          
         this.items = this.items.map(temp_item => {
    // Find matching item from all_items based on item_code
    const matched_item = all_items.find(item =>
        item.item_code.toLowerCase() === temp_item.item_code.toLowerCase()
    );

    // If a matching item is found, assign its UOMs to the temp_item
    if (matched_item) {
        temp_item.uoms = matched_item.item_uoms;
        temp_item['item_prices'] = matched_item.item_prices;
        temp_item['item_uoms'] = temp_item.uoms;
      }

    return temp_item; // Return the updated item
});
      }
        //vm.calc_item_price(item);
       // item.stock_qty = item.qty*item.conversion_factor;
       // 
       //this.update_item_detail_cr(data_value[0]);
      }
