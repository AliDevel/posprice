        ...
        <div v-if="is_cashback">
          ...
          >
            <v-col cols="6" v-if="!is_mpesa_c2b_payment(payment)">
              <v-text-field :key="payment.idx+'F'"
                dense
                outlined
            ...
            <v-col
              ...
              "
            >
              <v-btn  :key="payment.idx"
                block
                :id="payment.idx"
                class="payment-button" 
                :class="{ 'selected-payment': payment.idx=== this.selectedPaymentMode }"
                color="primary"
                dark
...
export default {
  ...
  methods: {
    back_to_invoice() {
      ...
      localStorage.setItem('paymentWindow', 'false');
...
if (this.pos_profile.website_url){
"Custom method: For synching between Chynar website: Gets sold item from POS and Sends to website."
const transformedArray = this.invoice_doc.items.map(item => ({
  product_code: item.item_code,
  MarketId: "3",
  incoming_price:  item.incoming_rate,
  stock: Math.floor(item.actual_qty-item.qty),
}));
const url = this.pos_profile.website_url;
console.log(url);
const makeAsyncPostRequest = async (data) => {
try {
...
// Use Promise.all to fire off all requests concurrently
Promise.all(transformedArray.map(item => makeAsyncPostRequest(item)))
.then(() => {
...
.catch(error => {
  console.error('Error:', error);
    ...
    set_full_amount(idx) {
        ...
        this.selectedPaymentMode = idx;
        let buttonColor = 'green';
      this.invoice_doc.payments.forEach((payment) => {
        payment.amount = payment.idx == idx ? this.invoice_doc.grand_total : 0;
      
                let buttontemp = document.getElementById(`${payment.idx}`);
              
        if (buttontemp) {
          buttontemp.setAttribute('style', `background-color: "" `);
    }
      });
      const button = document.getElementById(`${idx}`);
        if (button) {
          button.setAttribute('style', `background-color: ${buttonColor} !important`);
        } 
        //"Find amount field by Id and Change it "
        let fieldId = idx + "F";
        let amountField = document.getElementById(`${fieldId}`);
                amountField.focus();
    },
    set_rest_amount(idx) {
      this.invoice_doc.payments.forEach((payment) => {
    ...
    shortPay(e) {
      var isPaymentWindow = localStorage.getItem('paymentWindow');
      if(isPaymentWindow === 'true'){
      if(this.invoice_doc){
      if (e.key === 'F3' ) {
       e.preventDefault();
        this.submit();
    ...
    shortSelectMode(e) {
      ...
      if(this.invoice_doc){
       ...
       if (this.selectedPaymentMode > 1 ){
          let idx = this.selectedPaymentMode - 1;
          this.set_full_amount(idx);
       }
      }
      if (e.key === 'ArrowDown' ) {
        e.preventDefault();
        if (this.selectedPaymentMode < this.invoice_doc.payments.length ){
          let idx = this.selectedPaymentMode + 1;
          this.set_full_amount(idx);
       }
      }
  ...
  created: function () {
    this.$nextTick(function () {
      evntBus.$on('send_invoice_doc_payment', (invoice_doc) => {
        ...
        if (
          this.invoiceType === 'Order' ){
            console.log(this.is_credit_sale);
    ...
    document.addEventListener('keydown', this.shortSelectMode.bind(this));
    document.addEventListener('keydown', this.shortPay.bind(this));
    document.addEventListener('keydown', this.shortPayPrint.bind(this));
    document.addEventListener('keydown', this.shortCancel.bind(this));
  },

  destroyed() {
    document.removeEventListener('keydown', this.shortSelectMode);
    document.removeEventListener('keydown', this.shortPay);
    document.removeEventListener('keydown', this.shortPayPrint);
...
.selected-payment {
  /* Styles for the selected payment mode */
  background-color: blue;  /* Change to the desired color */
  color: white;  /* Change text color to match the background color */
}
</style>
